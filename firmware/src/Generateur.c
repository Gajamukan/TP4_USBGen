...
void  GENSIG_UpdatePeriode(S_ParamGen *pParam)
{
    ...
    uint32_t Nombreechantillons = (MAX_ECH * pParam->Frequence);
...
void  GENSIG_UpdateSignal(S_ParamGen *pParam)
{
    int test = 0; 
    //amplitude 
    Nbr_Tic = pParam->Amplitude * VAL_ECH /10000;
    ...
    for (EchNb = 0;EchNb < 100; EchNb++)
    {
        switch (pParam->Forme)
        {
            ...
            Result[EchNb] = (NBRMAX/2 -(((pParam->Offset)*(NBRMAX/20))/1000)+(((NBRMAX/2* (sin((EchNb*3.6)*(3.14/180) )))/10000)* (pParam->Amplitude)))  ;
                break ;

            case SignalCarre : //ok sa fonctionne
            if(EchNb > (MAX_ECH/2))
            {
                Result[EchNb] = NBRMAX/2 - (((pParam->Offset)*(NBRMAX/20))/1000) + (((NBRMAX/2)/10000)*(pParam->Amplitude)) ;
            }
            else 
            {
              Result[EchNb] = NBRMAX/2 - (((pParam->Offset)*(NBRMAX/20))/1000) - (((NBRMAX/2)/10000)*(pParam->Amplitude)) ;
            ...
            if (EchNb< 1)
            {
                Offset = -((pParam->Offset*VAL_ECH)/10000); 
            }
